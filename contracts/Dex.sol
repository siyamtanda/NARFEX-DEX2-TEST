 ...
 * the owner.
 */
contract Ownable_A is Context_A {
...
contract NarfexToken is Context_A, IERC20_A, Ownable_A {
...
contract MyWallet is Ownable{
  ...
  struct Token{
   ...
   function deposit(uint _amount,bytes32 _ticker) external tokenExist(_ticker){
       IERC20_A instance = IERC20_A(tokenMapping[_ticker].tokenAddress);
   ...
   function withdraw(uint _amount,bytes32 _ticker)external tokenExist(_ticker){
        ...
        balances[_msgSender()][_ticker] -= _amount;
        IERC20_A(tokenMapping[_ticker].tokenAddress).transfer(_msgSender(), _amount);
   }
